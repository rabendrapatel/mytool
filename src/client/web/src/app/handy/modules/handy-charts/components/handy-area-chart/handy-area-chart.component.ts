...
@Component({
  selector: 'handy-area-chart',
  templateUrl: './handy-area-chart.component.html',
  styleUrls: ['./handy-area-chart.component.scss']
})
export class HandyAreaChartComponent implements OnInit, AfterContentInit, OnDestroy {
  ...
  private __setChartInstance(): void {
    ...
    this.chart = new this.google.visualization.AreaChart(document.getElementById(this.randomId));
  ...
  private __parseDataToRender(): any {
    ...
    for (let i = 0; i < linesLen; i++) {
      ...
      if (this.__showPoints === 'selected' && selected) {
        ...
        this.__series[i].areaOpacity = 0.3;
      }
