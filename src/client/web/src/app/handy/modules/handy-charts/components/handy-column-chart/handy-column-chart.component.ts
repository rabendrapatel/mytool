...
@Component({
  selector: 'handy-column-chart',
  templateUrl: './handy-column-chart.component.html',
  styleUrls: ['./handy-column-chart.component.scss']
})
export class HandyColumnChartComponent implements OnInit, AfterContentInit, OnDestroy {
  ...
  private __stacked: boolean = false;
  @Input('stacked') public set stacked(is: boolean) {

    if (this.__stacked !== is) {
      this.__stacked = is;
      this.__drawTrigger.next();
    }
  ...
  private __setChartInstance(): void {
    ...
    this.chart = new this.google.visualization.ColumnChart(document.getElementById(this.randomId));
  ...
  private __parseDataToRender(): any {
    ...
    // Xaxis column
    dataTable.addColumn('string', data.xAxis.label);
    ...
    for (let i = 0; i < linesLen; i++) {
      ...
      this.__series[i] = {
        color,
        dataOpacity: typeof this.__selectedLineIndex === 'number' || typeof this.__selectedPointIndex === 'number' ? 0.3 : 1 ,
        pointsVisible: false
      }
